"use strict" /*
let itemArray = document.querySelectorAll('.categories>li');

let fn=(element)=>{
    element.forEach(el => {
        console.log(`Категория: ${el.firstChild.textContent.trim()}`);
        console.dir(`Количество: ${el.children[0].childElementCount}`);
    });
} 
fn(itemArray);
*/ /*
...
console.dir(list.firstElementChild.style);*/ /*
let createMovieCard= ()=>{
    
    const movie = document.createElement('div');
    const moovieChoose = document.querySelector('.movie')
    ...
    movieDescription.textContent=`Spanning the years 1945 to 1955, a chronicle of the fictional Italian-American Corleone crime family. When organized crime family patriarch, Vito Corleone barely survives an attempt on his life, his youngest son, Michael steps in to take care of the
    would-be killers, launching a campaign of bloody revenge.`;
    moovieChoose.after(movie)
    movie.append(img, movieBody);
    movieBody.append(movieTitle,movieDescription)
...
const createList =array=>{
   const fragment = document.createDocumentFragment();
    ...
    return list.append(...fragment.children)
}
...
let createBoxes = num => {
  ...
  for (let i = 0; i < num; i++) {
    ...
    innerBox.style.backgroundColor = `rgb(
        ${Math.floor(Math.random() * 256)},
        ${Math.floor(Math.random() * 256)},
        ${Math.floor(Math.random() * 256)})`;
    root.appendChild(innerBox);
  }
};
